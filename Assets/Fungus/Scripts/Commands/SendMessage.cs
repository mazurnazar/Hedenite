...
namespace Fungus
{
    ...
    public class SendMessage : Command
    {
        ...
        public void ReseiveMessage(string message)
        {
            _message.Value = message;
            messageTarget = MessageTarget.SameFlowchart;
            MessageReceived[] receivers = GameObject.FindObjectsOfType<MessageReceived>();
            if (receivers != null)
            {
                for (int i = 0; i < receivers.Length; i++)
                {

...
namespace Fungus
{
    ...
    public class SetVariable : Command, ISerializationCallbackReceiver
    {
        ...
        protected virtual void DoSetOperation()
        {
            ...
            if (anyVar.variable.GetType() == typeof(StringVariable))
            {
                AddTextFieldToJournal(anyVar.data.stringData.stringVal);
            }

            if (anyVar.variable.GetType() == typeof(BooleanVariable))
            {
                StartPuzzle();
            }
        }
        ...
        // Add text field to journal




        public delegate void JournalText(string variable, string text);
        public static event JournalText AddTextFieldEvent;

        public delegate void NextScene(string name);
        public static event NextScene MoveToScene;

        public delegate void Start();
        public static event Start StartPuzzleEvent;
        public void AddTextFieldToJournal(string text)
        {
            if (anyVar.variable.Key == "Scene") MoveToScene.Invoke(anyVar.data.stringData.stringVal);
            else AddTextFieldEvent?.Invoke(anyVar.variable.Key, text);
        }

        public void StartPuzzle()
        {
            if (anyVar.variable.Key == "Puzzle") StartPuzzleEvent?.Invoke();
        }
    }
}
